...
class DatabaseManager:
    ...
    def initialize_database(self):
        """Initialize complete database with all Phase 2.1 features"""
        self.connect()
        ...
        # Timer and session tables
        self._create_timer_tables()
        
        # Goals system tables
        self._create_goals_tables()
        
        # Exercise PDF tables
        self._create_exercise_tables()
    ...
    def _create_core_tables(self):
        """Create core tables (topics, PDFs)"""
        # Topics table
        self.cursor.execute("""
        ...
        # PDFs table with database storage
        self.cursor.execute("""
    ...
    def _create_exercise_tables(self):
        ...
        try:
            self.cursor.execute("""
                ...
                ADD CONSTRAINT fk_sessions_exercise_pdf 
                FOREIGN KEY (exercise_pdf_id) REFERENCES exercise_pdfs(id) ON DELETE CASCADE
            """)
        except psycopg2.errors.DuplicateObject:
            pass  # Constraint already exists
        except Exception as e:
            logger.warning(f"Could not add exercise PDF foreign key: {e}")
    
    def _create_performance_indexes(self):
        """Create all performance indexes"""
        indexes = [
            # Core tables
            "CREATE INDEX IF NOT EXISTS idx_pdfs_topic_id ON pdfs(topic_id)",
            ...
            # Exercise PDFs
            "CREATE INDEX IF NOT EXISTS idx_exercise_pdfs_parent_id ON exercise_pdfs(parent_pdf_id)",
            "CREATE INDEX IF NOT EXISTS idx_exercise_pdfs_content_hash ON exercise_pdfs(content_hash)",
        ]
    ...
    def update_reading_metrics(self, pdf_id=None, exercise_pdf_id=None, topic_id=None, 
        ...
        try:
            with self.transaction():
                ...
                logger.debug(f"Updated reading metrics")
    ...
    def get_daily_reading_stats(self, target_date):
    ...
    def cleanup_temp_files(self):
        ...
        pattern = os.path.join(temp_dir, 'studysprint_*')
